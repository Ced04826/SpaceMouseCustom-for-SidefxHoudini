...
def load_config():
    ...
    default_config = {
        ...
        'presets': {
            'translate': {'axis_mapping': {'pan_horizontal': 'x', 'pan_vertical': '-y', 'zoom': 'z'}},
            'rotate': {'axis_mapping': {'pan_horizontal': '-ry', 'pan_vertical': '-rx', 'zoom': 'z'}}
        },
                ...
                for key in default_config:
                    if key in loaded:
                        if isinstance(default_config[key], dict) and key != 'presets':
                            default_config[key].update({k: v for k, v in loaded[key].items() if not k.startswith('#')})
                        else:
    ...
    if 'fps_axis_mapping' not in loaded:
        default_config['fps_axis_mapping'] = dict(default_config.get('viewport_axis_mapping', {}))
    ...
    if 'fps_axis_multiplier' not in loaded:
        default_config['fps_axis_multiplier'] = dict(default_config.get('viewport_axis_multiplier', {}))
    ...
    if active in default_config.get('presets', {}):
        ...
        default_config['axis_mapping'] = default_config['presets']['translate']['axis_mapping']
...
def save_config(config):
    ...
    save_data = {
        ...
        "network_axis_multiplier": config.get('network_axis_multiplier', {
            "pan_horizontal": 1.0,
            "pan_vertical": 1.0,
            "zoom": 1.0
        }),
        "viewport_axis_mapping": config.get('viewport_axis_mapping', {
            "translate_x": "x", "translate_y": "y", "translate_z": "z",
            "rotate_x": "rx", "rotate_y": "ry", "rotate_z": "rz"
        }),
        "viewport_speed": {
        ...
        "viewport_axis_multiplier": config.get('viewport_axis_multiplier', {
            "translate_x": 1.0,
            "translate_y": 1.0,
            "translate_z": 1.0,
            "rotate_x": 1.0,
        ...
        "fps_axis_mapping": config.get('fps_axis_mapping', {
            "translate_x": "x", "translate_y": "y", "translate_z": "z",   
            "rotate_x": "rx", "rotate_y": "ry", "rotate_z": "rz"
        }),
        "fps_speed": {
        ...
        "fps_axis_multiplier": config.get('fps_axis_multiplier', {
            "translate_x": 1.0,
            "translate_y": 1.0,
            "translate_z": 1.0,
            "rotate_x": 1.0,
        ...
        "button_hotkeys": config.get("button_hotkeys", {
            "network": {
                "button_1": "none",
                "button_2": "none"
            },
        ...
        "auto_mode_switch": config.get("auto_mode_switch", {
            "enabled": False,
            "network_under_cursor": True
        }),
        "hover_refresh": config.get("hover_refresh", {
            "enabled": False,
            "hz": 30,
            "method": "win32",
            "jitter_px": 1,
        ...
        "deadzone": {
            "value": config.get('deadzone', {}).get('value', 15)
        },
        "network": {
            "port": config.get('network', {}).get('port', 19879)
        }
    }
...
def switch_preset(preset_name):
    ...
    if preset_name not in CONFIG.get('presets', {}):
        print(f"Preset '{preset_name}' not found. Available: {list(CONFIG['presets'].keys())}")
        return False
...
def _register_reader_pidfile():
    try:
        ...
        payload = {"pid": os.getpid(), "t_start_ns": time.time_ns(), "udp_port": UDP_PORT}
...
def find_spacemouse():
    ...
    for d in hid.enumerate():
        ...
        elif d.get('usage_page') == 1 and d.get('usage') in (4, 5, 6):  # Joystick, Game Pad, Multi-axis
            others.append(d)
...
def read_spacemouse_loop(houdini_pid=None):
    ...
    for i, d in enumerate(interfaces):
        print(f"  [{i}] {d['product_string']} - usage_page={d['usage_page']}, usage={d['usage']}")
    ...
    parent_pid = houdini_pid if isinstance(houdini_pid, int) and houdini_pid > 0 else None
    ...
    last_hello_send = 0.0
    try:
        hello = json.dumps({'type': 'hello', 'seq': 0, 't_send_ns': time.time_ns()}).encode()
    ...
    try:
        while True:
            ...
            if (nx != 0 or ny != 0 or nz != 0 or nrx != 0 or nry != 0 or nrz != 0) or buttons_changed:
                seq += 1
                msg = json.dumps({
                    'x': nx, 'y': ny, 'z': nz,
                ...
                if buttons_changed or (now - last_debug_print) >= debug_print_interval:
                    ...
                    if buttons_mask:
                        ...
                        parts = []
                        pressed_buttons = [bit + 1 for bit in range(0, 16) if (buttons_mask & (1 << bit))]
                        for button_number in pressed_buttons[:6]:
                            ...
                            if combo_str and combo_str.lower() not in ("none", "off", "disabled", "disable", "null"):
                        ...
                        btn_str = f" BTN:{int(buttons_mask):04X}{extra_str} " + " ".join(parts)
...
def start_receiver():
    ...
    class SpaceMouseReceiver(QtCore.QObject):
        ...
        def __init__(self):
            ...
            self.mode = CONFIG.get('mode', 'network')  # 'network', 'viewport', or 'viewport_fps'
            self.pan_speed = CONFIG['network_speed']['pan']
            self.zoom_speed = CONFIG['network_speed']['zoom']
            self.network_axis_multiplier = dict(CONFIG.get('network_axis_multiplier', {
                'pan_horizontal': 1.0,
                'pan_vertical': 1.0,
                'zoom': 1.0,
            }))
            ...
            self.axis_mapping = dict(CONFIG['axis_mapping'])  # Network mode axis mapping
            self.viewport_axis_mapping = dict(CONFIG.get('viewport_axis_mapping', {
                'translate_x': 'x', 'translate_y': 'y', 'translate_z': 'z',
                'rotate_x': 'rx', 'rotate_y': 'ry', 'rotate_z': 'rz'     
            }))
            self.viewport_axis_multiplier = dict(CONFIG.get('viewport_axis_multiplier', {
                'translate_x': 1.0,
                'translate_y': 1.0,
                'translate_z': 1.0,
                'rotate_x': 1.0,
            ...
            self.fps_translate_speed = fps_speed.get("translate", self.viewport_translate_speed)
            self.fps_rotate_speed = fps_speed.get("rotate", self.viewport_rotate_speed)
            self.fps_axis_mapping = dict(CONFIG.get("fps_axis_mapping", self.viewport_axis_mapping))
            self.fps_axis_multiplier = dict(CONFIG.get("fps_axis_multiplier", self.viewport_axis_multiplier))
            self.button_hotkeys = dict(CONFIG.get("button_hotkeys", {}))
            self.auto_mode_switch = dict(CONFIG.get("auto_mode_switch", {"enabled": False, "network_under_cursor": True}))
            self.hover_refresh = dict(CONFIG.get("hover_refresh", {"enabled": False, "hz": 30}))
            ...
            if result:
                ...
                self._sync_polling()
                print(f"SpaceMouseReceiver: Listening on port {UDP_PORT} (mode: {self.mode})")
            else:
        ...
        def _sync_polling(self):
            ...
            try:
                ...
                if self.mode in ("network", "viewport", "viewport_fps"):
            ...
            try:
                ...
                if isinstance(msg, dict) and msg.get("type") == "hello":
                    ...
                    try:
                        ...
                        self.socket.writeDatagram(payload, last_sender_host, int(last_sender_port))
                    except Exception:
                        pass
        ...
        def _qt_key_from_token(self, token):
            ...
            if name in ("`", "~") or upper in ("GRAVE", "BACKTICK", "QUOTELEFT", "TILDE"):
        ...
        def _hold_key_down(self, key, modifiers):
            ...
            self._post_key_event(self._hotkey_target(), QtCore.QEvent.KeyPress, key, modifiers)
        ...
        def _hold_key_up(self, key):
            ...
            if count == 1:
                ...
                self._post_key_event(self._hotkey_target(), QtCore.QEvent.KeyRelease, key, mods)
        ...
        def _win32_send_key(self, vk, is_down):
            ...
            try:
                if not hasattr(self, "_win32_sendinput_init"):
                    ...
                    try:
                        ...
                        pass
                    try:
                        self._win32_MapVirtualKeyW.argtypes = (wintypes.UINT, wintypes.UINT)
            ...
            try:
                ...
                inp = self._win32_INPUT(self._win32_INPUT_KEYBOARD, self._win32_INPUT_UNION(ki=ki))
                sent = int(self._win32_SendInput(1, ctypes.byref(inp), ctypes.sizeof(inp)))
                return sent == 1
            except Exception as e:
        ...
        def _combo_for_button(self, mode_map, button_number):
            if not isinstance(mode_map, dict):
                return None
            for key_name in (f"button_{button_number}", str(button_number), button_number):
        ...
        def _apply_button_hotkeys(self, msg, mode_override=None):
            ...
            mode = mode_override if mode_override is not None else getattr(self, "mode", "network")
            ...
            for bit in range(0, 32):
                ...
                existing = getattr(self, "_button_hold_bindings", {}).pop(button_number, None)
                if existing:
                    try:
            ...
            for bit in range(0, 32):
                if not (released & (1 << bit)):
                    continue
                button_number = bit + 1
                binding = getattr(self, "_button_hold_bindings", {}).pop(button_number, None)
                if not binding:
                    continue
                try:
        ...
        def _maybe_refresh_hover_preselect(self):
            ...
            try:
                ...
                if under_cursor is None or under_cursor.type() != hou.paneTabType.SceneViewer:
                    return
            except Exception:
            ...
            try:
                ...
                if method == "cursor":
                    try:
                        ...
                        self._last_error = f"HoverRefreshError(cursor): {type(e).__name__}: {e}"
                    return
                ...
                if method == "win32":
                    ...
                    try:
                        ...
                        hwnd_widget = root if root is not None else (target.window() if target is not None else target)
                        ...
                        user32.PostMessageW(hwnd, WM_MOUSEMOVE, wparam, lparam_from_point(hwnd_pos2))
                        user32.PostMessageW(hwnd, WM_MOUSEMOVE, wparam, lparam_from_point(hwnd_pos))
                    except Exception as e:
                        self._last_error = f"HoverRefreshError(win32): {type(e).__name__}: {e}"
                    return
        ...
        def _update_perf_metrics(self, msg, recv_ns=None, steps=1):
            ...
            if self._perf_latency_min_ms is None or latency_ms < self._perf_latency_min_ms:
                self._perf_latency_min_ms = latency_ms
            if self._perf_latency_max_ms is None or latency_ms > self._perf_latency_max_ms:
        ...
        def _update_perf_apply(self, apply_start_ns, apply_end_ns):       
            ...
            if self._perf_last_apply_end_ns is not None:
                interval_ms = (apply_end_ns - self._perf_last_apply_end_ns) / 1_000_000.0
        ...
        def _maybe_send_perf_reply(self, host, port):
            ...
            try:
                ...
                interval_avg = (sum(interval_window) / len(interval_window)) if interval_window else None
                apply_hz = (1000.0 / interval_avg) if interval_avg and interval_avg > 0 else None
        ...
        def _apply_viewport_input(self, msg, steps=1):
            ...
            new_trans = hou.Vector3(
                ...
                current_trans[2] + tz * translate_speed * tz_mul    # Forward/Back (tz positive = zoom in)
            )
            ...
            cam_rot_c2w = current_rot if self._viewport_cam_rot_is_c2w else current_rot.transposed()

            rot_pitch = hou.hmath.buildRotateAboutAxis(hou.Vector3(1, 0, 0), pitch_angle)
            ...
            new_rot = new_rot_c2w if self._viewport_cam_rot_is_c2w else new_rot_c2w.transposed()
        ...
        def _apply_viewport_fps_input(self, msg, steps=1):
            ...
            translate_speed = getattr(self, "fps_translate_speed", self.viewport_translate_speed) * 0.1
            rotate_speed = getattr(self, "fps_rotate_speed", self.viewport_rotate_speed) * 0.5
            if steps > 1:
                translate_speed *= steps
                rotate_speed *= steps

            axis_mul = getattr(self, "fps_axis_multiplier", getattr(self, "viewport_axis_multiplier", {}))
            tx_mul = axis_mul.get("translate_x", 1.0)
            ty_mul = axis_mul.get("translate_y", 1.0)
            tz_mul = axis_mul.get("translate_z", 1.0)
            rx_mul = axis_mul.get("rotate_x", 1.0)
            ...
            new_cam_pos_world = cam_pos_world + mul_vec3_mat3_row(delta_local, new_rot_c2w)

            # Solve translation that yields new camera position under the new rotation.
            new_trans = mul_vec3_mat3_row(new_cam_pos_world - pivot_world, new_rot_c2w.transposed())
            cam.setTranslation(new_trans)
            cam.setRotation(new_rot)
            viewport.setDefaultCamera(cam)
            self._maybe_refresh_hover_preselect()
        ...
        def stop(self):
            ...
            try:
                for _btn, binding in list(getattr(self, "_button_hold_bindings", {}).items()):
                    inject = binding.get("_inject", "qt")
                    if inject == "win32":
                        if binding.get("main_vk") is not None:
                            self._hold_vk_up(binding.get("main_vk"))
...
def reload_config():
    ...
    if receiver:
        ...
        receiver.network_axis_multiplier = dict(CONFIG.get("network_axis_multiplier", {}))
        ...
        receiver.viewport_axis_mapping = dict(CONFIG.get('viewport_axis_mapping', {}))  # Viewport mode
        receiver.viewport_axis_multiplier = dict(CONFIG.get("viewport_axis_multiplier", {}))
        fps_speed = CONFIG.get("fps_speed", {})
        receiver.fps_translate_speed = fps_speed.get("translate", receiver.viewport_translate_speed)
        receiver.fps_rotate_speed = fps_speed.get("rotate", receiver.viewport_rotate_speed)
        receiver.fps_axis_mapping = dict(CONFIG.get("fps_axis_mapping", receiver.viewport_axis_mapping))
        receiver.fps_axis_multiplier = dict(CONFIG.get("fps_axis_multiplier", receiver.viewport_axis_multiplier))
        ...
        print(f"  viewport_speed: translate={receiver.viewport_translate_speed}, rotate={receiver.viewport_rotate_speed}")
        print(f"  fps_speed: translate={receiver.fps_translate_speed}, rotate={receiver.fps_rotate_speed}")
...
def debug_receiver():
    ...
    if receiver:
        ...
        print(f"  axis_mapping (network): {receiver.axis_mapping}")       
        print(f"  network_axis_multiplier: {getattr(receiver, 'network_axis_multiplier', 'N/A')}")
        print(f"  viewport_axis_mapping: {getattr(receiver, 'viewport_axis_mapping', 'N/A')}")
        print(f"  viewport_axis_multiplier: {getattr(receiver, 'viewport_axis_multiplier', 'N/A')}")
        print(f"  fps_axis_mapping: {getattr(receiver, 'fps_axis_mapping', 'N/A')}")
        print(f"  fps_axis_multiplier: {getattr(receiver, 'fps_axis_multiplier', 'N/A')}")
        ...
        if hasattr(receiver, "_perf_latency_count"):
            try:
                ...
                interval_avg = None
                interval_window = list(getattr(receiver, "_perf_apply_interval_window_ms", []))
                if interval_window:
                ...
                print("=== Performance ===")
                print(f"  latency_ms: last={_fmt_ms(receiver._perf_latency_last_ms)} mean={_fmt_ms(lat_mean)} std={_fmt_ms(lat_std)} min={_fmt_ms(receiver._perf_latency_min_ms)} max={_fmt_ms(receiver._perf_latency_max_ms)}")
                print(f"  latency_ms_window: p50={_fmt_ms(p50)} p90={_fmt_ms(p90)} p99={_fmt_ms(p99)} (n={len(window)})")
                print(f"  backlog_steps: last={receiver._perf_steps_last} avg={_fmt_ms(steps_avg)} max={receiver._perf_steps_max} skipped_seq={receiver._perf_seq_skipped}")
                print(f"  apply_ms: last={_fmt_ms(receiver._perf_apply_last_ms)} avg={_fmt_ms(apply_avg)}")
                print(f"  apply_interval_ms: last={_fmt_ms(receiver._perf_apply_interval_last_ms)} avg={_fmt_ms(interval_avg)} hz={_fmt_ms(apply_hz)}")
            except Exception as e:
...
def set_mode(mode):
    """Set the control mode ('network', 'viewport', or 'viewport_fps')."""
    global _receiver, CONFIG

    if mode not in ('network', 'viewport', 'viewport_fps'):
        print(f"Invalid mode '{mode}'. Use 'network', 'viewport', or 'viewport_fps'.")
        return False
